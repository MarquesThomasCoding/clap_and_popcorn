...
import { fetchMovieById } from "@/lib/fetchData";
import { Crew, Movie } from "@/types/types";
...
export default async function Page({
  ...
  const movieData: Movie & { credits: { cast: Crew[]; crew: Crew[] } } = await fetchMovieById(slug);

  return (
    <>
      <MediaBanner media={movieData} type="movie" isMediaPage />
      <main className="flex flex-col gap-8 mx-20 p-8">
        <SeeAndShareButtons media={movieData} type="movie" />
        <section className="flex flex-col gap-4">
          <h2 className="text-3xl">Produit par</h2>
          <ul className="flex justify-between">
            {movieData.production_companies.map(
              (company: { id: number; name: string; logo_path: string }) => (
                <li key={company.id}>
                  {company.logo_path ? (
                    <Image
        ...
        <MediaListPreview
          medias={movieData.recommendations.results}
          type="movie"
          title="Similaire"
        />
      </main>
