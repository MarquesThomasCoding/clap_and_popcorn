...
export default function PersonPreview({ person }: { person: Person }) {
    return (
        <Link href={`/persons/${person.id}`} key={person.id} className="relative group overflow-hidden">
            ...
            <div className="absolute h-full w-full bottom-0 bg-black bg-opacity-50 opacity-0 translate-y-full group-hover:opacity-100 group-hover:translate-y-0 flex flex-col justify-end p-4 transition-all duration-500">
                ...
                <p>{isNaN(new Date(person.birthday).getTime()) ? '' : new Date(person.birthday).toLocaleDateString('fr-FR')}</p>
                <div className="flex items-center mt-2">
                    {Array.from({ length: 5 }, (_, index) => (
                        <svg
                            key={index}
                            className={`w-4 h-4 ${index < Math.round(person.popularity / 2) ? 'text-yellow-400' : 'text-gray-400'}`}
                            fill="currentColor"
                            viewBox="0 0 20 20"
